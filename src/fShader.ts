...
vec4 get_caustic2(vec2 uv ){
    ...
    float intensity = (n.w + 1.0) * 0.5;
    intensity = pow(intensity, 2.0);
    float a = pow(intensity, 1.0);
	return vec4(intensity, intensity, intensity, a);
   
}

vec4 get_foam(vec2 uv){
    float a = get_caustic2(uv).a - 0.5;
    vec2 f = uv + 0.015*vec2(a, a);
    float foamWeight = texture(u_channel2, f).a;
    vec4 foam = vec4(vec3(foamWeight), foamWeight * 0.25);
    return foam;
}
...
vec4 renderRipples() {
    ...
    // use u_channel2 to stop ripples near objects
    //float c2 = texture(u_channel2, uv).r;
    //d = mix(d, 0.5, c2*c2);
...
vec4 renderPass() {
    ...
    float a = 1.0 + dot(normalize(surfacePos - uv_down), normal);
    ...
    */
    ...
    vec4 env = texture2D(u_texture, rr);
    
    vec4 tex = env * 0.8;

    vec4 caustic = get_caustic2(uv);
    
    float caustic_light = 0.045;
    vec2 ccc =  vec2(caustic.a * caustic_light, caustic.a * caustic_light);
    
    vec4 lights = texture2D(u_environment, reflect_ray.xy + ccc);
    ...
    float total =  w_text + w_lights + w_caustic;
